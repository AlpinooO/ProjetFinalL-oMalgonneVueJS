<template>
    <div v-if="chef">
        ...
        <div v-if="chef.dishes && chef.dishes.length > 0">
            <div v-for="dish in chef.dishes" :key="dish.name" class="dish-card">
                <h3>{{ dish.name }}</h3>
...
onMounted(async () => {
    ...
    if (!isNaN(chefId)) {
        try {
            ...
            chef.value = chefs.find((c: any) => c.id === chefId);
        } catch (error) {
            console.error('Erreur lors du chargement des détails du cuisinier:', error);
        }
    } else {
        console.error("ID du chef non valide :", route.params.id);
    }
});
</script>
